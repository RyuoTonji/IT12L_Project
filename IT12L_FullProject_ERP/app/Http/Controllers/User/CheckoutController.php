...
class CheckoutController extends Controller
{
    ...
    public function process(Request $request)
    {
        ...
        // Save Address/Phone to Profile if requested
        $user = Auth::user();
        if ($request->has('save_address') && $request->save_address) {
            $user->address = $request->address;
        }
        if ($request->has('save_phone') && $request->save_phone) {
            $user->phone = $request->customer_phone;
        }
        if ($user->isDirty(['address', 'phone'])) {
            $user->save();
        }
    ...
    public function confirm(Request $request)
    {
        ...
        $order = DB::table('crm_orders')
            ->join('crm_branches', 'crm_orders.branch_id', '=', 'crm_branches.id')
            ->where('crm_orders.id', $orderId)
            ->where('crm_orders.user_id', auth()->id())
            ->select('crm_orders.*', 'crm_branches.name as branch_name', 'crm_branches.address as branch_address', 'crm_branches.phone as branch_phone')
            ->first();
