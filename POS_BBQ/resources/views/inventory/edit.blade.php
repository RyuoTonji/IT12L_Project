    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            ...
            <form action="{{ route('inventory.update', $inventory) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="category" id="category"
                            ...
                            <option value="Ingredients" {{ old('category', $inventory->category) == 'Ingredients' ? 'selected' : '' }}>Ingredients</option>
                            <option value="Prepared Menu" {{ old('category', $inventory->category) == 'Prepared Menu' ? 'selected' : '' }}>Prepared Menu</option>
                            <option value="Others" {{ old('category', $inventory->category) == 'Others' ? 'selected' : '' }}>
                                Others</option>
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="quantity" class="block text-sm font-medium text-gray-700 mb-1">Quantity (Current:
                            {{ $inventory->quantity }} {{ $inventory->unit }})</label>
                        <input type="number" name="quantity" id="quantity"
                            value="{{ old('quantity', $inventory->quantity) }}" step="0.01" min="0"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <p class="text-xs text-gray-500 mt-1">If you change this value, a stock adjustment record will be
                            created.</p>
                    </div>
