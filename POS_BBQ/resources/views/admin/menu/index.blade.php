...
@section('content')
        ...
        <div class="p-6 text-gray-900">
            ...
            <div class="mb-6">
                <div class="relative max-w-md">
                    ...
                    <input type="text" id="menuSearch" placeholder="Search menu items by name..."
                        class="pl-10 pr-4 py-2 w-full border border-gray-300 rounded-lg focus:ring-2 focus:ring-black focus:border-black"
            ...
            <div x-data="{ activeTab: 'cat-0' }">
                    ...
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        @foreach($menuItemsByCategory as $categoryName => $items)
                            <button @click="activeTab = 'cat-{{ $loop->index }}'"
                                ...
                                {{ $categoryName }}
                                <span
                                    class="bg-gray-100 text-gray-900 ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium md:inline-block">
                                    {{ $items->count() }}
                                </span>
                            </button>
                        @endforeach
                ...
                @forelse($menuItemsByCategory as $categoryName => $items)
                    <div x-show="activeTab === 'cat-{{ $loop->index }}'" class="mb-8 category-section"
                            ...
                            <table class="min-w-full bg-white divide-y divide-gray-200" id="table-cat-{{ $loop->index }}">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach($items as $item)
                                        ...
                                        @endphp
                                        <tr class="menu-item-row hover:bg-gray-50 transition-colors"
                                            ...
                                            <td class="py-4 px-6 text-center">
                                                <div class="inline-flex items-center gap-2 justify-center w-full">
                                                    <span id="is_available-label-{{ $item->id }}" class="flex items-center">
                                                    ...
                                                    <button onclick="showIsAvailableEdit({{ $item->id }})"
                                                ...
                                                <div id="is_available-edit-{{ $item->id }}"
                                                    class="hidden inline-flex items-center gap-2 justify-center w-full">
                                                    <select id="is_available-select-{{ $item->id }}"
                                                        class="rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 text-sm py-1">
                                                        <option value="1" {{ $item->is_available ? 'selected' : '' }}>Available
                                                        </option>
                                                        <option value="0" {{ !$item->is_available ? 'selected' : '' }}>Not Available
                                                        </option>
                                                    </select>
                                                    <button onclick="saveIsAvailable({{ $item->id }})"
                                                    ...
                                                    <button onclick="cancelIsAvailableEdit({{ $item->id }})"
                                            ...
                                            <td class="py-4 px-6 text-center">
                                                <div class="inline-flex items-center gap-2 justify-center w-full">
                                                    ...
                                                    <button onclick="showBranchEdit({{ $item->id }})"
                                                        class="text-gray-400 hover:text-blue-600 transition-colors">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                                            viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                ...
                                                <div id="branch-edit-{{ $item->id }}"
                                                    class="hidden inline-flex items-center gap-2 justify-center w-full">
                                                    <select id="branch-select-{{ $item->id }}"
                                                        class="rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 text-sm py-1">
                                                        <option value="branch1" {{ $branchStatus === 'branch1' ? 'selected' : '' }}>
                                                            Branch 1</option>
                                                        <option value="branch2" {{ $branchStatus === 'branch2' ? 'selected' : '' }}>
                                                            Branch 2</option>
                                                        <option value="both" {{ $branchStatus === 'both' ? 'selected' : '' }}>Both
                                                            Branches</option>
                                                    </select>
                                            ...
                                            <td class="py-4 px-6 text-center">
                                                <div class="flex justify-center items-center space-x-2">
                                                    <a href="{{ route('menu.show', ['menu' => $item, 'source' => 'menu']) }}"
                                                    ...
                                                    <form id="deleteMenuForm{{ $item->id }}"
                                                        action="{{ route('menu.destroy', $item) }}" method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
    ...
    <script>
        ...
        function showIsAvailableEdit(menuId) {
            document.getElementById(`is_available-label-${menuId}`).parentElement.classList.add('hidden');
            document.getElementById(`is_available-edit-${menuId}`).classList.remove('hidden');
        }

        function cancelIsAvailableEdit(menuId) {
            document.getElementById(`is_available-label-${menuId}`).parentElement.classList.remove('hidden');
            document.getElementById(`is_available-edit-${menuId}`).classList.add('hidden');
        }

        function saveIsAvailable(menuId) {
            const isAvailable = document.getElementById(`is_available-select-${menuId}`).value;
