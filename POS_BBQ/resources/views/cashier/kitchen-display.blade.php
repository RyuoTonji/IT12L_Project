                    ...
                    @foreach($orders as $order)
                        <div class="bg-white rounded-lg shadow border p-4 {{ $order->status == 'new' ? 'border-blue-500' : 'border-yellow-500' }}"
                            ...
                            <div class="flex justify-between items-center mb-2">
                                <h2 class="text-lg font-semibold">
                                    ...
                                    <span id="order-status-{{ $order->id }}"
                                        class="ml-2 px-2 py-1 text-xs rounded-full {{ $order->status == 'new' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800' }}">
                            ...
                            <div class="mt-4 flex justify-end">
                                @if($order->status == 'new')
                                    <button type="button"
                                        class="start-preparing-btn px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 flex items-center inline-flex"
                                ...
                                @elseif($order->status == 'preparing')
                                    <button type="button"
                                        class="mark-ready-btn px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600 flex items-center inline-flex"
                                        data-order-id="{{ $order->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                    ...
                    if (data.success) {
                        ...
                        if (statusElement) {
                            ...
                            if (status === 'preparing') {
                                ...
                                newButton.className = 'mark-ready-btn px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600 flex items-center inline-flex';
                                newButton.dataset.orderId = orderId;
                                newButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>Mark as Ready';
