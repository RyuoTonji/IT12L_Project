                ...
                @foreach($tables as $table)
                    @php
                        $isOccupied = \App\Models\Order::where('table_id', $table->id)
                            ...
                            ->exists();
                        $statusClass = $isOccupied ? 'bg-red-100 border-red-300' :
                            ($table->status == 'available' ? 'bg-green-100 border-green-300' : 'bg-yellow-100 border-yellow-300');
                        $statusLabel = $isOccupied ? 'Occupied' : ucfirst($table->status);
                    @endphp
                    <a href="{{ route('tables.show', $table) }}" class="block">
                        <div
                            class="aspect-square flex flex-col items-center justify-center rounded-lg shadow border {{ $statusClass }}">
